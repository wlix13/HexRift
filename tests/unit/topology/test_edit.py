...
class TestUnsplicableRegions:
    ...
    def test_empty_flow_hub_routes_are_editable(self):
        text = "regions:\n" + MSK_REGION + "routing:\n  hub_default: direct\n  hub_routes: []\n"
        assert Topology(text).remove_node("mskA00").text == text.replace(MSK_A00_BLOCK, "")


class TestAddNode:
    def test_appends_after_last_node_of_region(self):
        result = Topology(BASE).add_node(NL, NEW_NODE)
    ...
    def test_appends_under_bare_nodes_key(self):
        text = "regions:\n  - id: nl\n    type: exit\n    nodes:\n"
        result = Topology(text).add_node(NL, NEW_NODE)
    ...
    def test_rejects_region_type_mismatch(self):
        with pytest.raises(TopologyError, match="has type 'exit'"):
            Topology(BASE).add_node(NL_HUB, NEW_NODE)
...
class TestRemoveNode:
    ...
    def test_item_with_dash_on_its_own_line_spans_from_the_dash(self):
        tail = NL_TAIL.replace("      - id: nlA10\n", "      -\n        id: nlA10\n")
...
class TestRoundtrip:
    def test_add_then_remove_in_existing_region_restores_text(self):
        added = Topology(BASE).add_node(NL, NEW_NODE).text
